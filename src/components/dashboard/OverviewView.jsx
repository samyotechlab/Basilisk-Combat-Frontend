...
export function OverviewView({setCurrentView}) {
  const { token } = useAuth();
  ...
  const loadStats = async () => {
    try {
      ...
      setStats({
        ...
        winRate: analytics.victory_rate * 100 || 78.5,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-purple-300">
          Monitor your combat operations against the Digital Basilisk
        </p>
      </div>
...
function StatusItem({ label, status }) {
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <span
        className={`px-3 py-1 bg-${config.color}-500/20 text-${config.color}-300 rounded-full text-sm`}
      >
        {config.text}
      </span>
    </div>
...
function QuickActionCard({ title, description, icon: Icon, onClick, id }) {
  return (
    <div
      className="p-4 bg-slate-900/50 border border-purple-500/30 rounded-lg hover:border-purple-500/50 transition-all cursor-pointer"
      onClick={() => onClick(id)}
    >
